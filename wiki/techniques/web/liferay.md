...
title: Liferay
type: technique
tags: [exploitation, reference-import, web]
phase: exploitation
...
Liferay Portal exposes administrative portlets, a control panel, and a JSON web services API that can be abused for unauthenticated SSRF, remote code execution, and open redirects depending on the installed version and patch level. The control panel's Groovy script console, available to portal administrators, provides direct OS command execution on the server. Unauthenticated RCE vulnerabilities have been found in Liferay's JSON web services interface (e.g., CVE-2020-7961), allowing arbitrary Java deserialization without credentials.
