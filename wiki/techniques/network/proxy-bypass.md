...
title: Proxy Bypass
type: technique
tags: [bypass, evasion, proxy, reference-import, windows]
...
Corporate web proxies intercept and inspect HTTP/S traffic, blocking C2 communication and outbound attacker connections; attackers bypass them by discovering the proxy configuration (from the `DefaultConnectionSettings` registry key or WPAD/PAC files) and routing traffic through the proxy using the `HTTP_PROXY` environment variable or tool-level proxy settings. Proxy authentication is bypassed by relaying the victim machine's credentials through the proxy or by using the machine account's built-in NTLM pass-through authentication. Alternative protocols (DNS-over-HTTPS, ICMP tunneling, WebSocket upgrades) and domain-fronting are used when the proxy performs deep packet inspection on standard HTTP/S traffic.
